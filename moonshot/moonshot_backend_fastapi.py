...
def get_db_connection():
    # Mac 환경 로컬 DB 연결 (기본 설정)
    return psycopg2.connect(dbname="moonshot")

@app.on_event("startup")
def startup_event():
    # 서버 켜질 때 DB에 데이터가 없으면 JSON 파일을 읽어서 자동 주입
        ...
        try:
            ...
            for v in visa_list:
                ...
                """, (
                    ...
                    json.dumps(v.get("aliases", []), ensure_ascii=False), 
                    json.dumps(v.get("subCodes", []), ensure_ascii=False)
                ))
            conn.commit()
            print(f"[Moonshot DB] 성공적으로 {len(visa_list)}개의 데이터를 주입했습니다!")
        except Exception as e:
            print(f"[Moonshot DB] 데이터 주입 실패: {e}")
    else:
...
def get_visas():
    ...
    for r in rows:
        result.append({
            ...
            "extReq": r[7],
            "faq": r[8],
            "aliases": r[9] if r[9] else [],
            "subCodes": r[10] if r[10] else []
        })
